    ...
    void Update() {
        ...
        if (IsAutoAdvance)
        {
            if (!activeAudio.loop && activeAudio.clip != null)
            {
                ...
                {
					if (!loopPlaylist && currentPlaylist.fadeOutLastSong) {
						if (isShuffle) {
							...
							if (currentSequentialClipIndex >= currentPlaylist.MusicSettings.Count || currentPlaylist.MusicSettings.Count == 1) {
								FadeOutPlaylist();
								return;
							}
						}
